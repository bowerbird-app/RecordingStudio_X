...
class HooksTest < Minitest::Test
  def test_template_does_not_ship_a_copied_hooks_class
    ...
    refute defined?(RecordingStudio::X::Hooks)
  end
  ...
  def test_engine_runs_addon_hooks_through_configuration
    called = false
    RecordingStudio::X.configuration.hooks.after_initialize { called = true }

    initializer = RecordingStudio::X::Engine.initializers.find do |entry|
      entry.name == "recording_studio_x.after_initialize"
    end
